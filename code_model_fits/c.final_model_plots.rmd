...
fig1b <-ggplot() +
 ...
 # Theme and labels
  ...
  theme(
   ...
   # Facet by REALM (rows) and species (columns), but hide REALM labels
  facet_wrap(REALM ~ species, labeller = labeller(REALM = function(x) ""), scale = "free")

fig1b
```

```{r save}
figure_realm_NDVI_species <- (fig1a / fig1b)+ plot_layout(guides = "collect", axis_titles = "collect")

figure_realm_NDVI_species

ggsave(
  filename = "figure_realm_NDVI_species_level.png",
  plot = figure_realm_NDVI_species,
...
ggsave(
  ...
  plot = fig2,
  width = 9,       # in inches (adjust as needed)
  height = 7,      # in inches
  dpi = 600
)
```
...
```{r world species}
smoothed_ndvi_ribbons_world <- smoothed_ndvi_ribbons %>%
  group_by(species, world, dsl_wmNDVI) %>%
  summarize(
    fit_smooth   = mean(fit_smooth),
    lower_smooth = mean(lower_smooth),
    upper_smooth = mean(upper_smooth),
...
fig5 <- ggplot(smoothed_ndvi_ribbons_world) +
  geom_ribbon(
    aes(
      x = dsl_wmNDVI,
      ymin = lower_smooth,
  ...
  facet_wrap(world ~ species, labeller = labeller(world = function(x) ""), scale = "free") +
  theme_minimal()

fig5
```

```{r world species plot}
ggsave(
  filename = "figure_New_Old_species.png",
  plot = fig5,
  width = 9,       # in inches (adjust as needed)
  height = 7,      # in inches
  dpi = 600
