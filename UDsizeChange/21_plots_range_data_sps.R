...
dir.create(paste0(genPath,"land_ocean"))
pthlandocean <- paste0(genPath,"land_ocean/")
...
allrange <- rast(list.files("/home/ascharf/Documents/Projects/Drylands/UDsizeChange/A1.rastersOfAllSpeciesRange/", full.names = T))
plot(allrange[[1]])
sumallrange <- sum(allrange)
plot(sumallrange)

allpresrange <- rast(list.files("/home/ascharf/Documents/Projects/Drylands/UDsizeChange/A2.RasterDataOnRange/", full.names = T))
plot(allpresrange[[1]])
...
poly <- as.polygons(range1)
poly_sf <- st_as_sf(poly)
sumallrange[sumallrange == 0] <- NA
ggplot() +
  # geom_sf(data = ne_coastline(returnclass = "sf", 50), color="grey70") +
  geom_sf(data=ocean, fill="skyblue1", alpha=.4)+geom_sf(data=land, fill="white")+ #paleturquoise1 #bisque
  # geom_spatraster(data = range1) +
  geom_sf(data=poly_sf, aes(fill=sum), fill="grey80",show.legend = T)+
  # geom_sf_pattern(
  #   data = poly_sf,
  #   aes(fill = sum),
  #   fill="white",
  ...
  geom_spatraster(data = sumonlypres, aes(fill = sum)) +
  coord_sf(crs = crs(sumonlypres)) +
  scale_fill_whitebox_c(name="Nb species",palette="viridi", direction=1)+
  # scale_pattern_manual(name = "Total range", values = c("circle"))+
  # scale_fill_whitebox_c("Nb species (data)",palette="viridi", direction=-1,limits=c(0,17))+
  theme_bw()+
  theme(legend.position = "bottom")
ggsave("/home/ascharf/ownCloud - ascharf@ab.mpg.de@owncloud.gwdg.de/_quick_between/dryMartin/data_overlap_scalerange_wht.png",width = 30,height = 20, units="cm") 
