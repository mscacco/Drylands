...
### studies to download
sharedStudies <- readRDS(paste0(genPath,"table_shared_studies_20241125.rds"))
publicStudies <- readRDS(paste0(genPath,"table_public_studies_20241118.rds"))

all <- rbind(sharedStudies,publicStudies)
...
lastDwld <- "2024-11-18"
thisDwnl <- "2024-11-25" ## if interrupted
...
results <- lapply(Ids_toDo, function(studyId)try({
  ...
  reftb$pthName <- paste0(studyId,"_",reftb$individual_local_identifierNObsl,".rds")
...
###### for those studies that need accepting license terms
