...
# [1] "1252551761_E33w.rds"
# "1252551761_J15w.rds"
# [1] "1254061373_Dupla.rds"
# indvFileRDS <- "1254061373_Dupla.rds"
# doneInd <- list.files(pthmonthlydBBvar, full.names = F)
# flsMV2 <- flsMV[!flsMV%in%doneInd]
tictoc::tic()
results <- llply(flsMV, function(indvFileRDS)try({
  print(indvFileRDS)
...
dir.create(paste0(genPath,"7.vultureIndv_monthlyMotionVar"))
pthmonthlyMotionVar <- paste0(genPath,"7.vultureIndv_monthlyMotionVar/")

flsMV2 <- list.files(pthmonthlydBBvar, full.names = F)

tictoc::tic()
results <- llply(flsMV2, function(indvFileRDS)try({
  print(indvFileRDS)
...
tictoc::tic()
results <- lapply(flsMV5, function(indvFileRDS)try({
  print(indvFileRDS)
...
monthlyUDtable <- paste0(genPath,"11.vultureIndv_monthlyUDtable/")
